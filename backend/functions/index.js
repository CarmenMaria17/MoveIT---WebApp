...
exports.createReservation = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    if (!userReservationsOnDate.empty) {
      ...
      for (const doc of userReservationsOnDate.docs) {
        ...
        if (hoursOverlap(existingReservation.hour, hour)) {
          ...
          res.status(400).json({ 
            success: false, 
            error: `You already have a reservation at ${existingHour} on this date. Reservations cannot overlap or be adjacent.` 
          });
          return;
        }
      }
    ...
    if (!existingReservations.empty) {
      res.status(400).json({ success: false, error: 'This time slot is already reserved' });
      return;
    }
...
exports.getReservationsByCenterAndDateTime = functions.https.onRequest(async (req, res) => {
  // Enable CORS
  res.set('Access-Control-Allow-Origin', '*');
